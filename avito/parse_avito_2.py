        ...
        development_info = await page.evaluate('''
            () => {
                try {
                    ...
                    // Получаем весь текст из заголовка
...
async def click_apartment_type_button(page, button_info: Dict, apartment_type: str, room_filter_options: List[Dict]) -> str:
    """Кликает по header_marker элементу и перехватывает API запрос"""
    try:
        ...
        def handle_request(request):
            ...
            url = request.url
            if 'newDevelopmentsCatalog/development/items' in url and 'key=' in url:
        ...
        try:
            ...
            if captured_key:
                ...
                print(f"❌ Не удалось перехватить key")
                return None
...
async def fetch_apartment_data(page, key: str, room_type_id: str, apartment_type: str, limit: int = 100, offset: int = 0) -> Dict:
    ...
    try:
        # Формируем URL и body для запроса
        api_url = f"https://www.avito.ru/web/2/newDevelopmentsCatalog/development/items?key={key}"
        
        # Получаем текущий URL страницы для referrer
        current_url = await page.evaluate('() => window.location.href')
        base_url = current_url.split('?')[0]  # Убираем параметры
        
        request_body = {
            "url": f"{base_url}?limit={limit}&offset={offset}&roomsTypeIds[]={room_type_id}",
            "subdomainSlug": ""
        }
...
async def process_single_url(page, url: str, page_num: int, total_urls: int) -> Dict:
    ...
    try:
        ...
        if has_apartments:
            ...
            if not apartment_type_buttons:
                error_msg = "Кнопки типов квартир не найдены"
                print(f"❌ {error_msg}")
            ...
            # Кликаем ОДИН раз по первой кнопке для получения key
            api_key = None
            if sorted_buttons:
                ...
                if not api_key:
                    print(f"❌ Не удалось получить API key - требуется перезапуск браузера")
